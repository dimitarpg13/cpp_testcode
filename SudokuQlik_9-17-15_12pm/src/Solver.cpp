	 	...
	 	vector<rank_list *>::iterator itR;
	  	...
	  	for (itR = c.begin(); itR != c.end(); itR++)
	  	{
            ...
            for (itRL = (*itR)->begin(); itRL != (*itR)->end(); itRL++)
            {
            	...
            	if (!itRL->first->isEmpty())
            		cout << " = " << itRL->first->getValue();
            	cout << "\t{";
...
//                         }
		  ...
		  {
		     if (curSymbol != s && curSymbol->getValue() == s->getValue())
			 return false;
		  }
				...
				if (!curAssignments->empty())
				{
				   ...
				   { // while loop start
					  ...
					  if (curChar != 0)
					  {
						...
						if (iterCount == 11317)
						{
							// check [0,2] which then backtracks to [0,1] which then backtracks to NULL
							int ii=0;
							ii=1;
							print_ranked_candidates(m_vRankedCandidates);
						}
//
//						cout << "The value of [1,7] is: " << m_pSol->getRows()[1]->getSymbols()[7]->getValue() << endl;
//						cout << "The value of [2,6] is: " << m_pSol->getRows()[2]->getSymbols()[6]->getValue() << endl;
//
//						cout << "Symbol [1,7] has at present " << m_pSol->getRows()[1]->getSymbols()[7]->getAssignments()->size() << " assignments" << endl;
//						list<char> * ass = m_pSol->getRows()[1]->getSymbols()[7]->getAssignments();
//						for (list<char>::iterator it = ass->begin(); it != ass->end(); it++)
//						    cout << (*it) << " ";
//						cout << endl;
//
//						cout << "Symbol [2,6] has at present " << m_pSol->getRows()[2]->getSymbols()[6]->getAssignments()->size() << " assignments" << endl;
//						ass = m_pSol->getRows()[2]->getSymbols()[6]->getAssignments();
//						for (list<char>::iterator it = ass->begin(); it != ass->end(); it++)
//							cout << (*it) << " ";
//						cout << endl;
//
//						if (curSymbol->getRow()->getIdx()==2 && curSymbol->getCol()->getIdx()==6 &&
//							curSymbol->getValue()=='9' &&
//							m_pSol->getRows()[1]->getSymbols()[7]->getValue() == '9')
//						{
//
//							int iii=0;
//							iii=1;
//						}
//
//						if (curSymbol->getRow()->getIdx()==1 && curSymbol->getCol()->getIdx()==7)
//						{
//							int jjj = 0;
//
//							jjj=1;
...
//						cout << "Symbol [1,7] has at present " << m_pSol->getRows()[1]->getSymbols()[7]->getAssignments()->size() << " assignments" << endl;
//						ass = m_pSol->getRows()[1]->getSymbols()[7]->getAssignments();
//						for (list<char>::iterator it = ass->begin(); it != ass->end(); it++)
//							cout << (*it) << " ";
//						cout << endl;
//
...
//						cout << endl;
					 ...
					 if (!processed)
					 {
						 ...
						 // TO DO: restore_assignment seems to be called twice for symbols which did not find a feasible value
						 // in their assignment lists. This should be prevented from happening.
